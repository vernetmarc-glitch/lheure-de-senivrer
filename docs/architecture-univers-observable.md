...
**Pas encore fait** : aucune intégration en production (`DensityLayer.tsx`, `RealGalaxiesLayer.tsx`) — tout reste dans `app/public/*-test.html`, séparé de l'app réelle. L'ancrage résiduel permanent à `a=1` (§11.4.d) n'est pas encore implémenté, y compris dans les prototypes. La matrice de paramètres (§11.6) n'a pas encore été formellement documentée comme structure de données — chaque prototype calcule encore ses paramètres à la volée plutôt que de les lire depuis une matrice définie à l'avance.
