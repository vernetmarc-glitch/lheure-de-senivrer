...
- **19 juillet — exploration du générateur de structures (Ψ hiérarchique, points, particules, combo) : ABANDONNÉE, aucun résultat concluant.** Suite aux retours de Marc sur la v3.3 déployée (gros sauts de matière au changement de zoom, filaments ambiants mal échelonnés sur le Groupe Local, Voie lactée hires décentrée), plusieurs pistes ont été testées EN LOCAL, jamais commitées sur les fichiers de production : (1) coupure fine de Ψ en Mpc comobiles au lieu de pixels (correctif réel, gardé en note) ; (2) Ψ construit hiérarchiquement (héritage direct du parent + détail additif, cohérence du CHAMP prouvée 0.79-0.95) — mais la densité déposée (CIC) reste peu cohérente entre zooms (0.08-0.43) à cause du repliement non linéaire ; (3) génération par catalogue de points (crêtes de Hessienne, squelette) — rejetée visuellement ("mousse", "grains uniformes", pas de filaments longs) ; (4) particules advectées affichées directement (sans dépôt) — rejetée ("ciel étoilé", aucune structure) ; (5) combo dépôt+points superposés — jugé "pas mal" en test unitaire, mais le prototype de continuité de zoom (`app/public/zoom-continuity-test.html`, textures séparées, jamais raccordé à la production) a révélé une **grosse discontinuité entre layers et un effet de zoom qui ne fonctionne pas**. Décision de Marc : remettre à plat le générateur dans une NOUVELLE conversation, sans l'historique de cette exploration. **Tous les fichiers de production ont été explicitement rétablis à l'état v3.3 committé (`git checkout`) — aucune trace de cette exploration ne subsiste dans density_l*.png, spacetime_matrix.json, generate_layers.py ni zeldovich_engine.py.** Les scripts d'exploration restent dans `scripts/dev/` (`preview_v3_iter4.py` à `preview_row_AM_v2.py`, `bake_zoomtest_textures.py`) et le doc matrice §12.g, pour référence diagnostique uniquement — NE PAS reprendre leur code comme point de départ, repartir du principe plutôt que de l'implémentation.
